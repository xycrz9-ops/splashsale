------------------------------------------------------
-- == RIVALS ONLY GATE == --
-- Allows: Rivals public, 1v1/2v2 matches, private servers
-- Blocks: every other Roblox game
-- Re-execute allowed if menu is not actually open
------------------------------------------------------
do
    -- force re-run support
    if getgenv().NexisRivalsForceReload then
        getgenv().NexisRivalsForceReload = nil
        getgenv().NexisRivalsLoaded = nil
        pcall(function()
            if Library and Library.Unload then Library:Unload() end
        end)
    end

    -- only skip if menu is really open (not just a leftover flag from a failed load)
    local alreadyOpen = false
    pcall(function()
        if getgenv().Toggles and getgenv().Options and Library and not Library.Unloaded then
            alreadyOpen = true
            return
        end
        ...
        for _, gui in ipairs(CoreGui:GetChildren()) do
            if gui:IsA("ScreenGui") then
                ...
                if (n:find("nexis") or n:find("linoria")) and gui.Enabled ~= false then
                    alreadyOpen = true
                    break
                end
            end
        ...
        if gethui then
            for _, gui in ipairs(gethui():GetChildren()) do
                if gui:IsA("ScreenGui") then
                    local n = string.lower(tostring(gui.Name))
                    if n:find("nexis") or n:find("linoria") then
                        alreadyOpen = true
                        break
                    end
                end
    ...
    if alreadyOpen then
        getgenv().NexisRivalsLoaded = true
        return
    end

    -- clear stale flag so a previous failed execute can retry
    getgenv().NexisRivalsLoaded = nil

    -- wrong game: kick
    local RIVALS_UNIVERSE_ID = 6035872082
    local okGame, gameId = pcall(function() return game.GameId end)
    if not okGame or gameId ~= RIVALS_UNIVERSE_ID then
        pcall(function()
            local lp = game:GetService("Players").LocalPlayer
            if not lp then
                lp = game:GetService("Players").PlayerAdded:Wait()
            end
            lp:Kick("splash\nJoin Rivals You Dumbo\ndiscord.gg/KXAeyJkwGa")
        end)
        return
    end

    -- mark loaded only after game check passes; full menu load happens after this
    getgenv().NexisRivalsLoaded = true
end
------------------------------------------------------
-- == END RIVALS ONLY GATE == --
------------------------------------------------------
loadstring([[
...
do
    local t = 0
    while game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("LoadingScreen") and t < 3 do
        task.wait(0.1)
        t = t + 0.1
    end
end
...
local CUSTOM_UI_FONTS = {
    ...
    ['Tahoma'] = {
        ...
        AltFontFiles = { 'splash/Tahoma.font', 'library/Tahoma.font', 'Tahoma.ttf', 'library/Tahoma.ttf' },
    },
};
...
local Library = {
    ...
    AccentColor = Color3.fromHex("0096FF");
    OutlineColor = Color3.new(0, 0, 0);
    SelectedTabColor = Color3.fromHex("0a0a0a");
    GradientColor = Color3.fromHex("0096FF");
    ...
    OverlayGlowColor = Color3.fromHex('0096FF');
...
function Library:SetGradientText(Label, Text)
    ...
    local gradient = Label:FindFirstChild('splashTextGradient');
    if not gradient then
        gradient = Library:Create('UIGradient', {
            Name = 'splashTextGradient';
            Parent = Label;
        });
...
function Library:UpdateMenuBlur()
    ...
    local blur = lighting:FindFirstChild('splashMenuBlur');
...
function Library:SaveSoundSettings()
    ...
    local SettingsPath = (self.SoundSettingsFolder or 'splash') .. '/settings/sounds.json';
    local Data = {
...
function Library:InitSoundSettings(Folder)
    ...
    self.SoundSettingsFolder = Folder or 'splash';
...
function Library:InitSoundAssets()
    local Folder = self.SoundSettingsFolder or 'splash';
...
function Library:CreateWindow(...)
    ...
    do
        ...
        if not saved then
            pcall(function()
                if isfile and isfile('splash/ui_size.txt') then
                    local d = tostring(readfile('splash/ui_size.txt'));
    ...
    do
        ...
        local function persistSize()
            ...
            pcall(function()
                if writefile then
                    if makefolder and isfolder and not isfolder('splash') then makefolder('splash') end;
                    writefile('splash/ui_size.txt', math.floor(s.X) .. ',' .. math.floor(s.Y));
                end;
            end);
        end;
    ...
    local function UpdateTitle()
        local rawTitle = Config.Title or "splash";
...
local ThemeManager = {} do
	...
	ThemeManager.DefaultTheme = 'splash';
	ThemeManager.BuiltInThemes = {
		...
		['splash'] 	    = { 65, httpService:JSONDecode('{"FontColor":"e0d4d6","MainColor":"121212","AccentColor":"0096FF","SelectedTabColor":"0a0a0a","BackgroundColor":"0a0a0a","OutlineColor":"000000","GradientColor":"0096FF"}') },
	};
	...
	function ThemeManager:LoadDefault()
		local theme = self.DefaultTheme or 'splash';
...
local SaveManager = (function()
    ...
    SaveManager.Folder = 'splash/rivals';
    SaveManager.Ignore = {};
...
local Window = Library:CreateWindow({
    Title = 'Splash Client   discord.gg/DcWVQXGKtp',
    AutoShow = false,
    BackgroundImage = "",
    SubTitle = "Rivals",
    Center = true,
...
task.spawn(function()
    for name, suffix in font_files do
        local ttf_path = "splash/rivals/fonts/" .. suffix;
        local font_path = "splash/rivals/fonts/" .. name .. ".font";
  ...
  "FIntEnableCullableScene2HundredthPercent3": "100"
    ...
    getgenv().elisium_inject_fflags = function(json_str)
        ...
        if writefile then
            pcall(function()
                if makefolder and isfolder and not isfolder("splash") then makefolder("splash") end;
                writefile("splash/fflags.json", json_str);
                saved = true;
            end);
        end;
...
do
    function skybox_changer:init()
        ...
        self.list = {
            ...
            ["splash Sky"] = { SkyboxBk = "rbxassetid://1898724755", SkyboxDn = "rbxassetid://1898727189", SkyboxFt = "rbxassetid://1898722814", SkyboxLf = "rbxassetid://1898729298", SkyboxRt = "rbxassetid://1898741025", SkyboxUp = "rbxassetid://1898736761" },
        };
...
task.spawn(function()
    while true do
        task.wait(ELI.guns.rapid_fire_rate or 0.05);
        if ELI.guns.rapid_fire then
            pcall(function()
                local local_fighter = getgenv().local_fighter or (getrenv and getrenv().shared and getrenv().shared.local_fighter);
                if local_fighter and local_fighter.Input then
                    local_fighter:Input("StartShooting");
                end;
            end);
        end;
...
silentaim_tab:AddSlider('silent_aim_lerp', {
 ...
 aimbot_tab:AddSlider('aimbot_lerp', {
    ...
    local guns_tab = legit_tab:AddRightGroupbox('guns'); do
        ...
        guns_tab:AddToggle('rapid_fire_enable', {
            Text = 'rapid fire',
            Default = false,
            Callback = function(v)
                ELI.guns.rapid_fire = v;
                if Options.rapid_fire_rate then
                    Options.rapid_fire_rate:SetVisible(v);
                end;
            end
        });

        guns_tab:AddSlider('rapid_fire_rate', {
            Text = 'fire rate',
            Default = 0.05,
            Min = 0.01,
            Max = 0.2,
...
do
    ...
    ff_box:AddLabel('2 it applies live if your executor supports it and always saves them to the splash fflags file', true);
    ...
    local function do_inject(json)
        ...
        elseif res.saved then
            Library:Notify('fflags saved to splash/fflags.json - import in your executor & rejoin', 7);
        else
    ...
    ff_box:AddButton('save preset to file', function()
        ...
        pcall(function()
            if makefolder and isfolder and not isfolder('splash') then makefolder('splash') end;
            writefile('splash/fflags.json', getgenv().elisium_fflag_preset or '{}');
        end);
        Library:Notify('saved preset to splash/fflags.json', 5);
    end);
end;
...
local misc_tab = Tabs.misc_tab; do
    ...
    local elo_spoof = spooftabthing:AddTab('Others Ranks');
...
local function openCosmeticChanger(showUi)
    ...
    local SKINS_FOLDER = "splashrivals_skins";
...
;(function()
    ...
    local crosshair_box = crosshair_tab:AddTab("crosshair"); do
        ...
        crosshair_box:AddToggle("CrosshairJewMode", {
            Text = "jew mode",
            Default = false,
            Callback = function(v)
                cfg.crosshair.jew_mode = v;
                -- Hide normal crosshair options when jew mode is on
                local hide = v;
                pcall(function()
                    if Options.CrosshairStyle then Options.CrosshairStyle:SetVisible(not hide) end;
                    if Options.CrosshairLines then Options.CrosshairLines:SetVisible(not hide) end;
                    if Options.CrosshairRotation then Options.CrosshairRotation:SetVisible(not hide) end;
                    if Options.CrosshairLength then Options.CrosshairLength:SetVisible(not hide) end;
                    if Options.CrosshairThickness then Options.CrosshairThickness:SetVisible(not hide) end;
                    if Options.CrosshairGap then Options.CrosshairGap:SetVisible(not hide) end;
                end);
            end
        });
...
do
    ...
    -- Moved to table to avoid local register limit (200)
    Library.__StyleNotification = function()
        local Area = GetNotifyArea();
        if not Area then return end;
        ...
        for _, Outer in eli_ipairs(Area:GetChildren()) do
            if Outer:IsA("GuiObject") and not Outer:GetAttribute("ElisiumStyled") then
                ...
                if Visible < 1 then
                    local function applyFade(Object)
    ...
    if not Library.__ElisiumNotifyHooked then
        ...
        function Library:Notify(...)
            ...
            task.defer(Library.__StyleNotification);
            return Data;
        end;
    end;
...
ThemeManager:SetFolder('splash');
SaveManager:SetFolder('splash/rivals');
SaveManager:BuildConfigSection(Tabs['UI Settings'])

;(function()
    local lua_tab = Tabs.lua;
    local base_folder = SaveManager.Folder or 'splash/rivals';
...
;(function()
    local BRAND_ACCENT = Color3.fromHex('0096FF');
    ...
    pcall(function()
        AppearanceWidget = ThemeManager:BuildAppearanceOverlay({ Visible = true });
